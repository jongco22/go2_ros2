...
class Go2LaunchConfig:
    ...
    def _get_config_paths(self) -> dict:
        ...
        return {
            ...
            'nav2': os.path.join(self.package_dir, 'config', 'nav2_params_map3.yaml'),
...
class Go2NodeFactory:
    ...
    def create_include_launches(self) -> List[IncludeLaunchDescription]:
        ...
        return [
            ...
            # path_tracker: track_fast
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource([
                    os.path.join(get_package_share_directory('path_tracker'),
                                'launch', 'track_fast.launch.py') # 씨발 이거 실패하면 오른쪽걸로 바꿔야함 track_3_4.launch.py
                ]),
                launch_arguments={
                    'use_sim_time': use_sim_time,
                }.items(),
