...
class MissionController(Node):
    def __init__(self):
        ...
        self.declare_parameter('state', None)
        self.declare_parameter('mission_trigger_track3', None)

        mission_param = self.get_parameter('mission').value
        state_param = self.get_parameter('state').value
        mission_trigger_track3_param = self.get_parameter('mission_trigger_track3').value

        # Required mission (e.g., 'track_1', 'track_2', 'track_3', 'track_4')
        self.mission = mission_param
        self.state = state_param
        self.mission_trigger_track3 = mission_trigger_track3_param

        if self.mission_trigger_track3 is not None:
    ...
    def timer_callback_main(self):
        ...
        if self.mission == 'track_3' and self.state == None:
            ...
            if self.robot_navigation_state:
                ...
                self.mission_trigger_track3 = "first"
                return
            self.robot_navigation_publisher.publish(String(data='path_3_1'))
        ...
        elif self.mission == 'track_3' and self.state == 'robot_stand':
        ...
        elif self.mission == 'track_4' and self.state == 'robot_stand':
            self.get_logger().info('Track 4: robot stand')
            if self.robot_stand_state:
                self.state = 'driving'
            self.robot_stand_publisher.publish(String(data="stand"))
                
        elif self.mission == 'track_4' and self.state == 'driving':
            self.get_logger().info('Track 4: driving')
            if self.robot_navigation_state:
                self.mission = None
                self.state = None
            self.robot_navigation_publisher.publish(String(data='path_4'))
