...
private:
    ...
    void control_loop()
    {
        ...
        if (distance_error < distance_threshold_ && angle_error < angle_threshold_)
        {
            ...
            {
                current_pose_idx_++;
                RCLCPP_INFO(this->get_logger(), "Target reached! Moving to next pose #%zu", current_pose_idx_);
            ...
            else
            {
                ...
                std_msgs::msg::Bool feedback_msg;
                feedback_msg.data = true; // "Path completed successfully"
                navigation_feedback_pub_->publish(feedback_msg);
